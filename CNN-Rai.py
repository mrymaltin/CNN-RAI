...
def rai(X,k):
    ...
    k_1_vec = np.ones([len(k_vec),(4**(k-1))])
    k_1_vec=np.array(k_1_vec,dtype=float)    
    for i in range(len(k_vec)):
        for a in range(4**(k-1)):
            k_1_vec[i,a] = k_vec[i, a*4:a*4+4].sum()

    k_1_vec=k_1_vec/k_1_vec.sum()

    RAI = np.zeros([len(k_vec),(4**k)])

    RAI=np.array(RAI,dtype=float) 
    temp=np.min(k_vec[np.nonzero(k_vec)])
    tempk=np.min(k_1_vec[np.nonzero(k_1_vec)])
    for a in range(len(k_vec)):   
        for i in range(4**(k-1)):
            for j in range(4):
                ...
                if(k_vec[a, i*4+j]==0 ):
                    k_vec[a, i*4+j]=temp
                RAI[a,i*4+j] = 11*np.log(k_vec[a, i*4+j]) + np.log(k_1_vec[a, i])       
